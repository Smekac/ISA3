...
public class AdminFanController {
   ...
   //     RegPosetilacModel registrovani = regPosetilacService.findOne(id);
    ...
    @RequestMapping(
            value = "/promenaSifre/{sifra}",     // da promeni svoje podatke !!!!
            method = RequestMethod.POST,
            produces = MediaType.APPLICATION_JSON_VALUE
    )
    public ResponseEntity<AdminFanModel> promenaSifre( @PathVariable("sifra") String sifra ) {
        ServletRequestAttributes attr = (ServletRequestAttributes) RequestContextHolder.currentRequestAttributes();
        HttpSession session= attr.getRequest().getSession(true);
        Korisnik ref = (Korisnik) session.getAttribute("korisnik");

        AdminFanModel adi = adminFanService.findOne(ref.getId());

        adi.setPassword(sifra);

        adminFanService.save(adi);
        return new ResponseEntity<AdminFanModel>((AdminFanModel) ref,HttpStatus.OK);
    }
